      ...
      {!activeTrip && pendingTrips && pendingTrips.length > 0 && (
        <div className="bg-amber-50 border border-amber-200 rounded-xl px-5 py-4 mb-6 flex items-center justify-between gap-4">
          <p className="text-sm text-amber-800 font-medium">
            Tenés <span className="font-bold">{pendingTrips.length}</span> reparto{pendingTrips.length > 1 ? 's' : ''} pendiente{pendingTrips.length > 1 ? 's' : ''} de iniciar.
          </p>
          <Link to="/trips" className="text-xs font-semibold text-amber-700 hover:text-amber-900 underline shrink-0">
            Ver viajes →
          </Link>
        </div>
      )}
