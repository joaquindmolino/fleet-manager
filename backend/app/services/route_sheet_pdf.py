...
def build_route_sheet_pdf(data: RouteSheetData) -> bytes:
    ...
    elements.append(Paragraph(
        ...
        "Tocá el link de cada parada para navegar con Google Maps",
        footer,
    ))
...
def _build_stop_block(
    *,
    ...
    """Genera un bloque (tabla) para una parada con link a Google Maps.

    Layout:
      [N°] [alias + dirección + link 'Cómo llegar' + ETA + observaciones]
    """
    maps_url = GOOGLE_MAPS_DIR_URL.format(lat=lat, lng=lng)

    link_style = ParagraphStyle(
        "MapsLink", parent=stop_addr_style, fontSize=10, leading=12,
        textColor=HexColor("#1d4ed8"),
    )

    # Columna central: alias, dirección, link, ETA, observaciones
    ...
    middle_flowables.append(Paragraph(
        f'<link href="{maps_url}"><u>📍 Cómo llegar (Google Maps)</u></link>',
        link_style,
    ))
    info_bits: list[str] = []
    if eta_text:
    ...
    # Número de parada como chip de color
